...
class ChatbotFramework():
    ...
    def __init__(self, extracted_keywords: list, chatbot_mode='intent_detection'):
        ...
        self.intent_response = {
            ...
            'qa_mode': "What movie do you want to know more about? Type reset/quit to quit the QA Mode",
        }
    ...
    def convo(self, input):
        ...
        if self.chatbot_mode == 'intent_detection':
            ...
            if len(intent_detected) == 0:
                ...
                for intent in intent_detected:
                    if intent == "hello":
                        ...
                        # Going into the movie pipeline
                        response_list.append(self.intent_response['qa_mode'])
            ...
            if self.reset_convo(input) == False:
                ...
                self.recommended_movies = respond_results
            else:
            ...
            # Check if user wants to reset
            if self.reset_convo(input) == False:
                self.qa_model = self.qa_question_chatbot(input)
                respond_results = ["We have loaded the data for " + input]
                respond_results.append("Please ask your question")
                self.chatbot_mode = "qa_questions"
        ...
        elif self.chatbot_mode == "qa_questions":

            if self.reset_convo(input) == False:
                self.answer_list = self.qa_model.question_answer_chatbot(input)
                self.answer_state = 0
                first_answer = self.qa_model.answer_generator_chatbot(self.answer_list, self.answer_state)
                respond_results = [first_answer]
                respond_results.append("Is this answer satisfactory? (Y/N)")
                self.chatbot_mode = "qa_answers"
            else:
                # Go back to intent detection
                self.tracker = 0
                self.chatbot_mode = 'intent_detection'
        ...
        elif self.chatbot_mode == "qa_answers":

            if self.reset_convo(input) == False:
                # Checking the state of chatbot_answers
                if input == "N" or input == "n":
                    self.answer_state += 1
                    answer = self.qa_model.answer_generator_chatbot(self.answer_list, self.answer_state)

                    while answer == "":
                        self.answer_state += 1
                        answer = self.qa_model.answer_generator_chatbot(self.answer_list, self.answer_state)

                    if answer == "Oops There is no more answer":
                        self.answer_state = 0
                        self.chatbot_mode = "qa_questions"
                        respond_results = ["Please ask another question or type reset to start over!"]
                        return respond_results

                    respond_results = [answer]
                    respond_results.append("Is this answer satisfactory? (Y/N)")
                    ...
                    respond_results = ['Please answer the question!!']
    ...
    def get_input_respond_message_for_movie_recommender(self, input: str):
        ...
        list_of_prompt_response = [
            ...
            {'prompt_themes': ['reply_searching', 'reply_list_of_movies', "reply_qa"]},
